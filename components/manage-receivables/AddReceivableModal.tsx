      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm">
        ...
        >
          <div className="flex justify-between items-center mb-4 md:mb-6">
            <h2 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white">Tambah Peminjam Baru</h2>
          ...
          <div className="space-y-4 md:space-y-5">
            ...
            <div>
              <label className="block text-[10px] md:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 md:mb-2">Nama Peminjam</label>
              <input 
                type="text" 
                value={newName}
            ...
            <div>
              <label className="block text-[10px] md:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 md:mb-2">Kategori Piutang</label>
              <div className="grid grid-cols-2 gap-2">
                {(['B2B', 'Keluarga', 'Rekan Kerja', 'Ikut Transaksi'] as const).map((type) => (
                  <button
                    key={type}
                    onClick={() => setNewType(type)}
                    className={`p-2.5 md:p-3 rounded-xl border text-[9px] md:text-[10px] font-bold uppercase tracking-wider transition-all flex items-center justify-center space-x-2 ${
                      newType === type 
                  ...
                  >
                    <span className="shrink-0">{getCategoryIcon(type)}</span>
                    <span className="truncate">{type === 'B2B' ? 'Bisnis' : type}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="pt-2 md:pt-4">
              <button 
                onClick={handleAdd}
                className="w-full py-3.5 md:py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold rounded-xl md:rounded-2xl hover:bg-slate-800 dark:hover:bg-slate-100 transition-all shadow-lg shadow-slate-200 dark:shadow-none text-sm md:text-base"
              >
                Simpan Peminjam
              </button>
            </div>
