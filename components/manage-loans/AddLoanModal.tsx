      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm">
        ...
        >
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">Tambah Pinjaman Baru</h2>
          ...
          <div className="space-y-5">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Nama Kreditur</label>
              <input 
                type="text" 
                value={newName}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Jenis Pinjaman</label>
              <div className="grid grid-cols-2 gap-2">
                {(['Bank', 'Personal', 'CC/Paylater', 'Lainnya'] as const).map((type) => (
                  <button
                    key={type}
                    onClick={() => setNewType(type)}
                    className={`p-3 rounded-xl border text-[10px] font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 ${
                      newType === type 
                  ...
                  >
                    <span className={newType === type ? 'text-white' : 'text-slate-400 dark:text-slate-500'}>
                    ...
                    {type === 'CC/Paylater' ? 'Kartu Kredit' : type}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="pt-4">
              <button 
                onClick={handleAdd}
                className="w-full py-4 bg-rose-600 dark:bg-rose-500 text-white font-bold rounded-2xl hover:bg-rose-700 dark:hover:bg-rose-600 transition-all shadow-lg shadow-rose-100 dark:shadow-none"
              >
                Simpan Pinjaman
              </button>
            </div>
